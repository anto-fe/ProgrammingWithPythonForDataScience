...
#
# .. your code here ..
textfile = pd.read_csv('../Module5/Datasets/wheat.data', sep=',')
X = pd.DataFrame(textfile)
...
y.wheat_type = y.wheat_type.astype("category", ordered=True).cat.codes
...
#
# .. your code here ..
ndata_train = Normalizer().fit_transform(data_train)
ndata_test = Normalizer().fit_transform(data_test)
...
pca.fit(ndata_train)
X_train = pca.transform(ndata_train)
#label_test = pca.transform(label_test)
ndata_test = pca.transform(ndata_test)
#label_train = pca.transform(label_train)
#
...
print("Accuracy:",knn.score(ndata_test, label_test))
